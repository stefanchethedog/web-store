...
import { Schema as SchemaModel } from './schemaModel';

const compiledModels: any = {};

async function createItemModel(schemaName: string): Promise<typeof Model> {
  try {
...
async function createNestedItemModel(schemaName: string) {
  try {
    if (compiledModels[schemaName]) {
      return compiledModels[schemaName];
    }
